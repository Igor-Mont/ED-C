...
bool juntar(LISTA *l1, LISTA *l2, LISTA *l_juncao)
{
    ...
    int tamanhoFinal = t1 + t2;
  if(tamanhoFinal > MAX) return false;

  inicializar(l_juncao);

  for(int i = 0; i < t1; i++) {
    if(busca(l1->itens[i].chave, l_juncao) != -1) return false;
    inserirNaOrdem(l1->itens[i], l_juncao);
  }

  for(short int i = 0; i < t2; i++) {
    if(busca(l2->itens[i].chave, l_juncao) != -1) return false;
